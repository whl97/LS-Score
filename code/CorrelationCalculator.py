...
class CNNDM_Reader:
	...
	def _read_human_score(self, human_score_file):
		prompt_list = ['hter', 'overall', 'grammer', 'redundancy']
		_human_scores = read_cnndm_human_score(human_score_file)
		human_scores = {}
		...
		for prompt in prompt_list:
			...
			for id_key in human_scores:
				temp =  [scores_for_one_summary[prompt_index] for scores_for_one_summary in human_scores[id_key]]
				temp.pop(0) 
				human_scores_different_prompt[prompt][id_key] = temp
		return human_scores_different_prompt

	def _read_articles(self, articles_file, human_scores):
		a_prompt = list(human_scores.keys())[0]
		human_scores = human_scores[a_prompt]

		_articles = read_cnndm_articles(articles_file)
		articles = {id_key:_articles[id_key] for id_key in _articles if id_key in human_scores}
		return articles

	def _read_summaries_and_reference(self, summaries_file, human_scores):
		a_prompt = list(human_scores.keys())[0]
		human_scores = human_scores[a_prompt]
		_summaries, _ =  read_cnndm_summaries_and_reference(summaries_file)
		summaries = {}
		references = {}
	...
	def read_data(self):
		...
		summaries, references = self._read_summaries_and_reference(summaries_file, human_scores)
		articles = self._read_articles(articles_file, human_scores)

		data = {'human_scores':human_scores, 'summaries':summaries, 'references':references, 'articles':articles}
		return data




class Newsroom_Reader:
	def __init__(self, dataset_dir):
		self.dataset_dir = dataset_dir
	...
	def _read_human_score(self, human_score_file):
		...
		human_scores_different_prompt = {}
		for prompt in prompt_list:
			human_scores_different_prompt[prompt] = self._get_human_scores_prompt(human_scores, prompt=prompt)
		return human_scores_different_prompt
	...
	def _read_articles(self, articles_file, human_scores):
		...
		a_prompt = list(human_scores.keys())[0]
		human_scores = human_scores[a_prompt]
		articles = {key:art for key, art in _articles.items() if key in human_scores}
		return articles

	def _read_summaries_and_reference(self, references_file, human_score_file, human_scores):
	...
	def read_data(self):
		...
		human_scores = self._read_human_score(human_score_file)
		summaries, references = self._read_summaries_and_reference(references_file, human_score_file, human_scores)
		articles = self._read_articles(articles_file, human_scores)
...
class ModelEvaluator:
	def __init__(self, dataset_name, model_save_dir, load_model_name, pretrained_model_dir, model_type=None, is_cuda=True):
		...
		self.model_save_dir = model_save_dir + "/model_save_{}".format(self.dataset_name)
	...
	def _cal_moverscore(self, references, summaries, n_gram):
		...
		score_filename = 'other_metrics/moverscore/{}/ngram_{}_results'.format(self.dataset_name, n_gram)
		if os.path.exists(score_filename):
			...
			scores = {article_id_list[i]:scores[i*num_systems:(i+1)*num_systems] for i in range(len(article_id_list))}
	...
	def _cal_sms(self, references, summaries, dataset_name, embedding_type, metric_type):
		...
		score_filename = "other_metrics/sms/{}_ref_hyp_{}_{}.out".format(dataset_name, embedding_type, metric_type)  # todo
		...
		if os.path.exists(score_filename):
			...
			scores = {id_list[i]:score_list[i*num_systems:((i+1)*num_systems)] for i in range(len(id_list))}
	...
	def _cal_bleu_and_meteor(self, summaries, references, metric_name):
		...
		scores = {article_id_list[i]: scores[i*num_systems: (i+1)*num_systems] for i in range(num_of_articles)}
	...
	def cal_our_scores(self, summaries, articles, references, load_from_file=True, save_to_file=True):
		filename = self.score_save_dir+"/our_scores_{}.pkl".format(self.load_model_name)
		...
		if load_from_file and os.path.exists(filename): 
			...
			if save_to_file:
				with open(filename, 'wb') as handle:
						pickle.dump(our_scores, handle, protocol=pickle.HIGHEST_PROTOCOL)
	...
	def get_results(self):
		...
		for score_name, scores in three_our_scores.items():
			...
			for prompt in human_scores:
				...
				corr = self.corr_cal.get_correlation(scores, human_scores[prompt])
				...
				for metric_name in metric_list:
					if "rouge" in metric_name:
						for rpf_flag in "rpf":
							...
							corr = self.corr_cal.get_correlation(scores, human_scores[prompt])
							my_print("{}-{}:  {}".format(metric_name, rpf_flag, corr))

					elif "bleu" in metric_name or "meteor"==metric_name:
						scores = self._cal_bleu_and_meteor(summaries, references, metric_name)
						corr = self.corr_cal.get_correlation(scores, human_scores[prompt])
						my_print("{}:  {}".format(metric_name, corr))

					elif metric_name=="bert-score":
						for rpf_flag in "rpf":
							scores = self._get_bertscore_Single_metric(rpf_flag=rpf_flag)
							corr = self.corr_cal.get_correlation(scores, human_scores[prompt])
							my_print("{}-{}:  {}".format(metric_name, rpf_flag, corr))
						...
						for embedding_type in ['glove', 'elmo']:
							for metric_type in ['sms', 'wms', 's+wms']:
								...
								if scores != None:
									corr = self.corr_cal.get_correlation(scores, human_scores[prompt])
									my_print("{}-{}:  {}".format(metric_type, embedding_type, corr))


					elif metric_name == "moverscore":
						for n_gram in [1, 2]:
							scores = self._cal_moverscore(references, summaries, n_gram)
							if scores !=None:
								corr = self.corr_cal.get_correlation(scores, human_scores[prompt])
								my_print("{}-{}:  {}".format("moverscore", n_gram, corr))
...
if __name__ == "__main__":
	...
	def list_all_model(dataset_name):
		model_save_dir = MY_CONFIG.output_dir+"/model_save_{}".format(dataset_name)
		model_list = []
		dic_lis = [i for i in os.listdir(model_save_dir)]
		if len(dic_lis) == 0:
			raise FileNotFoundError("can not find any state dict in {}!".format(model_save_dir))
	...
	for dataset_name in dataset_list:
		...
		for model_name in model_list:
			print('-'*100)
			print("dataset_name:{}			 model_name:{}".format(dataset_name, model_name))
			...
			if evaluator.model_exits():
				evaluator.display_results(display_all_metrics=False, write_to_txt=False)
