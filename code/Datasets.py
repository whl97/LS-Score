...
def read_dataset(dataset_name, dataset_dir):
    if dataset_name == "cnndm":
        articles, references, summaries, human_scores = read_cnndm(dataset_dir["cnndm"])
    elif dataset_name == "newsroom":
        articles, references, human_scores = read_newsroom(dataset_dir["newsroom"])
        summaries = None
    articles, references, summaries = filter_out_test_data(human_scores, articles, references, summaries)

    return articles, references


def filter_out_test_data(human_score, articles, references, summaries):
    def func(x):
...
class Features(object):
    def __init__(self, input_ids=None, segment_ids=None, input_mask=None, mlm_labels=None, is_next_label=None):
        ...
        self.is_next_label = is_next_label
...
class RankingLossDataset(Dataset):
    def __init__(self, dataset_name, dataset_dir, tokenizer, train_or_test):
        ...
        self.articles, self.references = read_dataset(dataset_name, dataset_dir)



        self.neg_func_list = [self.delete_words, self.disorder_words, self.add_sentences]

        directory = os.path.dirname(os.path.abspath(__file__))
        self.cached_file = os.path.join(directory, 'cache/{}/cached_ranking_loss_negative_sampling'.format(dataset_name)) 
        self.sample_rate = 1
        self.train_proportion = 0.90
        self.train_or_test = train_or_test
        

        self.ART_SUM_PAIR_LIST = self.get_feature_lists(self.cached_file, self.train_or_test, self.train_proportion)
    ...
    def get_feature_lists(self, cached_file, train_or_test, train_proportion=0.95):
        if os.path.exists(self.cached_file):
            ...
            art_sum_pairs_list = self._process_dataset_and_save(self.articles, self.references, self.tokenizer, cached_file)
    

        num_samples = int(len(art_sum_pairs_list)*self.sample_rate)
        sample_index = random.sample(list(range(len(art_sum_pairs_list))), num_samples)
        random.shuffle(sample_index)
        art_sum_pairs_list = [art_sum_pairs_list[i] for i in sample_index]

        art_sum_pairs_list = self._split_train_and_test(art_sum_pairs_list, train_or_test, train_proportion)
    ...
    def _process_dataset_and_save(self, articles, references, tokenizer, cached_file):
        ...
        for idx, art_id in enumerate(articles):
            if art_id in references:
                ...
                art_input_ids, art_segment_ids, art_input_mask = self.get_features_from_tokens(tokenizer, art_tokens, max_seq_length=512)
                art_features = {"input_ids":art_input_ids, "segment_ids":art_segment_ids, "input_mask":art_input_mask}


                reference_tokens = tokenizer.tokenize(reference)
                reference_and_negative_sampling_tokens = [reference_tokens]
                reference_and_negative_sampling_tokens += [func(article, reference, tokenizer) for func in self.neg_func_list]

                n_art_sum_pairs = []


                for sum_tokens in reference_and_negative_sampling_tokens:
                    sum_input_ids, sum_segment_ids, sum_input_mask = self.get_features_from_tokens(tokenizer, sum_tokens, max_seq_length=512)
                    sum_features = {"input_ids":sum_input_ids, "segment_ids":sum_segment_ids, "input_mask":sum_input_mask}
        ...
        with open(cached_file, 'wb') as handle:
            pickle.dump(art_sum_pairs_list, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def delete_words(self, article, summary, tokenizer, probability=0.2):
        ...
        for i in range(num_words_to_delete):
            ...
            if is_subtoken(positive_tokens[index]):
    ...
    def add_sentences(self, article, summary, tokenizer, add_sent_num=1):
        ...
        for sent in sentences_in_summary:
            most_similar_sent = find_most_similar_sentence_in_article(sentences_in_article, sent)
    ...
    def disorder_one_sent(self, sent, probability):
        ...
        a_index_list, b_index_list = get_random_list(word_list, num_words_to_disorder)
