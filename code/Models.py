...
class BertForLS_Score(BertPreTrainedModel):
    ...
    def L_Score(self, input_ids, input_mask, sum_seq_output):  
        ...
        score = torch.sum(score.mul(one_hot_input_ids))/(torch.sum(input_mask).float())
        score = (score+200)/100
        return score
    ...
    def forward(self, art_sum_pairs):
        ...
        art_sum_input_ids = torch.tensor([article_input_ids]+summaries_input_ids).to(self.device)
        art_sum_segment_ids = torch.tensor([article_segment_ids]+summaries_segment_ids).to(self.device)
        art_sum_input_mask = torch.tensor([article_input_mask]+summaries_input_mask).to(self.device)
        ...
        for i in range(num_samples):
            ...
            l_scores[i] = self.L_Score(sum_i_input_ids, sum_i_input_mask, sum_seq_output)
